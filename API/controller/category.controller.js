...
export var updatecategory=async (req,res,next)=>{
  var ctDetails= await ctSchemaModel.findOne({_id:req.body._id});
  if(ctDetails){
  var id=req.body._id;
  delete req.body._id;
  var category= await ctSchemaModel.updateOne({_id:id},{$set:req.body});
  if(category)
...
export var deleteCategory = async (req,res,next)=>{
  ...
  var ctDetails= await ctSchemaModel.find({_id:id});
  var l = ctDetails.length;
  if(l!=0){
    let result= await ctSchemaModel.deleteMany({_id:id});
    if (result)
...
export var fetch = async (req,res,next)=>{
  var condition_category=req.body._id;
  var ctList= await ctSchemaModel.findOne(condition_category);
  var l = ctList.length;
  if(l!=0)
  res.status(201).json({"result":ctList});
  else
  res.status(500).json({error:"server error"});
}
